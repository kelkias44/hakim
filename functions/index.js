...
exports.onCreatePost = functions.firestore
  .document("/cases/{userId}/cases/{postId}")
  .onCreate(async (snapshot, context) => {
...
exports.onCreateComment = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    // 1) Get all the followers of the user who made the post
    const caseData = admin
      .firestore()
      .collection("cases")
      .doc(commentCreated.ownerId)
      .collection("cases").doc(postId).get();
    ...
    followers.forEach(id =>{
      admin
        ...
        .collection("feedItems").add({
          'type':'CaseFollowing',
          'userName':"",
          'uid':commentCreated.ownerId,
          'postId':postId,
          'mediaUrl':"",
          "commentData":commentCreated.content,
          'timestamp': new Date(),
        });
    });
...
exports.changeLastMessage = functions.firestore
      ...
      .onUpdate((change, context) => {
        ...
        if (message) {
          ...
          .collection("rooms")
          .doc(context.params.roomId).update({
